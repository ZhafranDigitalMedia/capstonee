...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setLoading(true);
      try {
        const results = await Promise.all(moods.map((m) => fetchMovies(m.query, 0)));

        setMoodMovieCount(moods.map((m, i) => ({ name: m.nama, total: results[i].length })));
        ...
        // Rating distribution
        const ratingMap: Record<string, number> = { "<6": 0, "6–7": 0, "7–8": 0, "8–9": 0, "9–10": 0 };
        unique.forEach((movie) => {
        ...
        setRatingDist(Object.entries(ratingMap).map(([name, value]) => ({ name, value })));

        // Top 5
        setTopMovies(
          [...rated].sort((a, b) => parseFloat(b.imdbRating) - parseFloat(a.imdbRating)).slice(0, 5)
        );
      } catch (e) {
        console.error(e);
      } finally {
    ...
    <main className="min-h-screen pt-24 bg-gradient-to-b from-[#0b1026] to-[#6a11cb] text-white">
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-400">Statistik penggunaan MoodFlix dan distribusi data film</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
              {[
                { icon: Film, label: "Total Film", value: totalFilms, color: "text-pink-400" },
                { icon: Star, label: "Avg Rating", value: avgRating, color: "text-yellow-400" },
                { icon: Smile, label: "Total Mood Dipilih", value: totalMoodClicks, color: "text-purple-400" },
                { icon: TrendingUp, label: "Total Genre", value: genreStats.length, color: "text-green-400" },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                <h3 className="text-xl font-semibold mb-1">😊 Mood Terpopuler</h3>
                <p className="text-gray-400 text-sm mb-6">Berdasarkan pilihan kamu</p>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={moodStats}>
                    ...
                    <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                      {moodStats.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                {totalMoodClicks === 0 && (
              ...
              {/* Genre Distribution */}
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                <h3 className="text-xl font-semibold mb-1">🎬 Distribusi Genre</h3>
                <p className="text-gray-400 text-sm mb-6">Top 8 genre dari seluruh film</p>
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={genreStats} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={85} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
              ...
              {/* Rating Distribution */}
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                <h3 className="text-xl font-semibold mb-1">⭐ Distribusi Rating IMDB</h3>
                <p className="text-gray-400 text-sm mb-6">Sebaran rating film yang direkomendasikan</p>
                <ResponsiveContainer width="100%" height={220}>
              ...
              {/* Film per Mood */}
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                <h3 className="text-xl font-semibold mb-1">🎭 Jumlah Film per Mood</h3>
                <p className="text-gray-400 text-sm mb-6">Film tersedia di tiap kategori mood</p>
                <ResponsiveContainer width="100%" height={220}>
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm lg:col-span-2">
                <h3 className="text-xl font-semibold mb-1">🏆 Top 5 Film Rating Tertinggi</h3>
                <p className="text-gray-400 text-sm mb-6">Film terbaik dari seluruh kategori mood</p>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                  {topMovies.map((movie, i) => (
                    <div key={movie.imdbID} className="flex flex-col items-center text-center gap-2">
                      <div className="relative w-full">
                        <img src={movie.Poster} alt={movie.Title} className="w-full h-40 object-cover rounded-xl" />
                      ...
                      <p className="text-yellow-400 text-sm font-bold">⭐ {movie.imdbRating}</p>
                    </div>
                  ))}
                </div>
              </div>
